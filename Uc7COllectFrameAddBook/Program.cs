    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            while (result == 1)
            {
                ...
                data.People = service.AddAddrBook(addrName, data.People);
                int loop = 1;
                while (loop == 1)
                {
                    ...
                    switch (option)
                    {
                        case 1:
                           ...
                           if(data.AddContact(FirstName, LastName, Address, City, State, ZipCode, PhoneNumber, Email))
                            {
                        ...
                        case 2:
                            ...
                            SetContact c = data.FindContact(name);
                            if (c == null)
                            {
                ...
                service.pairs[addrName] = (data.People);
                Console.WriteLine("Do you want to enter an address book. \n1. yes \n2. no");
                result = int.Parse(Console.ReadLine());
            }

            foreach (var key in service.pairs.Keys)
            {
                Console.WriteLine(key);

                foreach (SetContact c in service.pairs[key])
                {
